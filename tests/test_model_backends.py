...
def test_default_transports_and_cache_identity_unchanged(monkeypatch):
    ...
    assert (
        ...
        == "claude-frames-v1:api:" + generate.CLAUDE_MODEL
    )
    assert (
        ModelFrameJudge("claude-cli").cache_key
        == "claude-frames-v1:claude-cli:configured-cli-default"
    )
    assert (
